...
<head>
    ...
    <title><?= htmlspecialchars(__t('ui.x.declarer_panne_lpc')) ?></title>
...
<a href="#main" class="lpc-skip-link"><?= htmlspecialchars(__t('ui.a11y.skip_to_content')) ?></a>
    ...
    <main role="main" id="main" class="flex-1 overflow-y-auto p-4 pb-8">
        <div class="bg-red-50 border border-red-200 p-4 rounded-2xl mb-4 text-center">
            ...
            <p class="text-xs font-bold text-red-800"><?= htmlspecialchars(__t('ui.x.ceci_alertera_immediatement_la_logistiqu')) ?></p>
        </div>

        <form id="form-breakdown" class="bg-white rounded-2xl shadow-sm p-6 space-y-5" onsubmit="event.preventDefault(); submitBreakdown();">
            
            <div>
                <label class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5"><?= htmlspecialchars(__t('ui.x.mon_vehicule')) ?></label>
                <select id="maint_vehicle_id" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm font-bold text-gray-900 outline-none focus:ring-2 focus:ring-red-500">
                    <option value=""><?= htmlspecialchars(__t('ui.x.chargement')) ?></option>
                </select>
            </div>

            <div>
                <label class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5"><?= htmlspecialchars(__t('ui.x.odometre_actuel_km')) ?></label>
                <div class="relative">
            ...
            <div>
                <label class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5"><?= htmlspecialchars(__t('ui.x.description_du_probleme')) ?></label>
            ...
            <div>
                <label class="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5"><?= htmlspecialchars(__t('ui.x.cout_depannage_si_paye_sur_place')) ?></label>
                <div class="relative">
