 ...
 * -----------------------------------------------------------------------------
 * DESCRIPTION: Mobile interface for clients to verify empties returned and sign
 * with a drawn signature on their phone. Full spec: docs/SIGNATURES.md.
 *
 * WHAT CHANGED FROM THE SPRINT 7C VERSION
 *   The phone-OTP gate is gone. Customers now sign directly — draw, tap
 *   Confirmer et Signer, done — exactly like the Feb version, but on top of
 *   today's hardened plumbing (CSRF, i18n, cache-busted assets, forced
 *   light theme for PDF rendering, bottle_size/has_cork classification from
 *   migration 014).
 *
 *   The wire target is the unified /api/v1/signatures_controller.php with
 *   action=sign_external. That is the ONE endpoint every external signature
 *   goes through, regardless of doc type — no more per-page controllers.
 * -----------------------------------------------------------------------------
 */
        ...
        <div id="reject_modal" class="hidden absolute inset-0 bg-gray-900/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl p-6 w-full shadow-2xl">
                ...
                <div class="flex gap-3">
                    ...
                    <button id="reject_confirm_btn" type="button" class="flex-1 bg-rose-600 text-white py-3 rounded-xl font-bold text-sm shadow-md">Refuser le CRE</button>
                </div>
            </div>
        </div>
