...
<main id="main" role="main" class="w-full">
    ...
    <script type="application/json" id="lpc-page-data"><?= json_encode([
        ...
        'csrf'            => Csrf::token(),
        'csrfField'       => '_csrf',
        // Second receipt button — commented out, not deleted. This first sent
        // "Télécharger le reçu PDF" straight to the dompdf-rendered "offre
        // commerciale" (?pdf=1); once that was swapped for the html2canvas
        // WYSIWYG autodownload URL it became identical to "Télécharger le
        // document signé" below — two buttons doing the same thing. Client-
        // facing request: exactly one button, and it must autodownload. To
        // restore a second button, uncomment one of the two lines below.
        // 'successRedirect' => '/quote.php?token=' . $token . '&pdf=1',           // dompdf one-pager
        // 'successRedirect' => '/quote.php?token=' . $token . '&autodownload=1', // WYSIWYG (was here most recently)
