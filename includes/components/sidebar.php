...
<aside id="lpc-sidebar"
       ...
       class="fixed lg:static top-0 left-0 z-40 w-72 h-screen bg-white/5 backdrop-blur-2xl border-r border-white/10
    ...
    <div class="p-5 border-b border-white/10 flex items-center gap-3">
        <img src="/assets/img/small_logo.svg"
             ...
             alt="LPC" class="w-10 h-10 rounded-full">
        <div class="min-w-0">
            ...
            <ul role="menu" class="space-y-0.5">
                ...
                <li role="none">
                    <a href="<?= htmlspecialchars($item['href']) ?>"
                       ...
                       class="lpc-focusable group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all
                              <?= $active
    ...
    <div class="p-4 border-t border-white/10 flex items-center gap-3">
        <?php if ($avatar): ?>
            <img src="<?= htmlspecialchars($avatar) ?>" alt="" class="w-10 h-10 rounded-full object-cover ring-2 ring-emerald-400/40">
        <?php else: ?>
            <div class="w-10 h-10 rounded-full bg-emerald-500/30 text-white grid place-items-center font-semibold text-sm ring-2 ring-emerald-400/40">
        ...
        <a href="/api/v1/auth.php?logout=true" class="text-white/70 hover:text-red-400 p-2" title="<?= $lang==='en'?'Log out':'Se déconnecter' ?>" aria-label="<?= $lang==='en'?'Log out':'Se déconnecter' ?>">
